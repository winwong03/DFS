...
func double_write(serverAddr, localIP, localPath1 string, localPath2 string) (err error) {
	...
	testCase1 = fmt.Sprintf("Able to read '%s' back from chunk %d", "Written by B", 3)
...
func main() {
	...
	if err := double_write(serverAddr, localIP, clientALocalPath, clientBLocalPath); err != nil {
		reportError(err)
	}
